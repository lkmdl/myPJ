...
window.addEventListener("load", () => {
    ...
    *************************************************/
    const goSlide = (dir) =>{ // dir - 방향구분
        ...
        // 1.함수호출 확인
        console.log("나야!!! 나!!",dir);

        // 2. 방향에 따른 분기
        if(dir){ // 1이면 true이므로 오른쪽 방향
            snum++;
            if(snum===5) snum=0;// 한계값 설정 -> 처음으로
        }////if////

        else{ // 0이면 0이므로 왼쪽방향
            snum--;
            if(snum===-1) snum=4;// 한계값 설정 -> 끝으로
        }/////else////
        ...
        slide.style.transition = "left .6s ease-in-out";
